...
from FlexController_Jason import FlexController, DummyFlexController
...
class CalibrationUI:
    ...
    def __init__(self, flex_controller):
        ...
        self.steps = [
            "Foot at REST position",
            "Flex foot UP as much as you can",
            "Flex foot DOWN as much as you can",
            "Flex foot IN as much as you can",
            "Flex foot OUT as much as you can",
    ...
    def load_calibration(self):
        ...
        try:
            ...
            # Convert dictionary into a list of thresholds, ensuring 4 values exist
            self.thresholds = [calibration_data.get(f"Sensor_{i}", 0) for i in range(4)]
            print(f"Loaded calibration data: {self.thresholds}")

        except FileNotFoundError:
            print("Calibration configuration file not found. Using default thresholds.")
            self.thresholds = [0] * 4  # Default values in case the file is missing
        except json.JSONDecodeError:
            print("Error decoding calibration file. Using default thresholds.")
            self.thresholds = [0] * 4  # Default values in case of corruption
...
# self.flex_controller.current_sensor is defined as the current sensor being calibrated. Needs to trigger FOUR TIMES.
    ...
    def draw(self, screen, sensor_values):
        ...
        instruction = font_small.render(self.steps[step_index], True, YELLOW)
        ...
        if sensor_values and len(sensor_values) >= 4:
            ...
            for i in range(4):
                ...
                # Draw labels
                threshold = self.thresholds[i] if i < len(self.thresholds) else 0
