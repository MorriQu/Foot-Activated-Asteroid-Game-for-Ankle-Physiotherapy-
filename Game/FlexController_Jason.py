...
class FlexController:
    ...
    def save_calibration(self, threshold, current_sensor):
        """Save calibration configuration."""
        try:
            # Load existing calibration data if the file exists
            try:
                with open(CALIBRATION_FILE, 'r') as f:
                    calibration_data = json.load(f)
            except (FileNotFoundError, json.JSONDecodeError):
                calibration_data = {}  # Create a new dictionary if file doesn't exist or is corrupted

            # Update the specific sensor's threshold
            calibration_data[f"Sensor_{current_sensor}"] = threshold

            # Save updated calibration data
            with open(CALIBRATION_FILE, 'w') as f:
                json.dump(calibration_data, f, indent=4)

            print(f"Saved Sensor {current_sensor} Threshold: {threshold} to {CALIBRATION_FILE}")
        ...
        if self.current_sensor < 4:
            ...
            if elapsed >= 3:  # 3 seconds per calibration round    
                ...
                if self.cal_round > 2:  # 3 rounds complete for this sensor
                    ...
                    self.save_calibration(threshold, self.current_sensor)
    ...
    def sensor_outputs(self, threshold_multipler=0.95):
        ...
        up_thresh, down_thresh, left_thresh, right_thresh = [t * threshold_multipler for t in self.thresholds]
        
        # Calculate just how far above the (95% of)threshold each sensor is
        ...
        # Creates a dictionary active sensors and their respective strength
        ...
        if active_sensors:
            strongest_sensor = max(active_sensors.items(), key=lambda x: x[1])[0]
        ...
        if up_strength > 0 and left_strength > 0 and up_strength > down_strength:
            outputs[0] = 1
            outputs[2] = 1
